...
func main() {
	...
	flag.BoolVar(&syncCF, "sync-cf", false, "sync Cloudflare DNS A records to the current stored IP")
	...
	if store {
		...
		if strings.TrimSpace(dbname) == "" {
			...
			if err != nil {
				fmt.Fprintln(os.Stderr, "store error: cannot determine default db:", err)
				os.Exit(1)
			}
			dbname = d
		}
